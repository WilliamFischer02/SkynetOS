...
`docs/09-ASSET-CATALOGUE.md` — generated by `npm run assets:catalogue`. Every PNG under
`assets/vendor/`, measured: dimensions, grid, how many cells contain any non-transparent pixel,
colour count, ink coverage, and the licence line from each pack's SOURCE.md.

It is generated rather than written because a vendor README describes the pack that was published,
not the file sitting in this repo. A grid the tool inferred from image dimensions is marked
INFERRED and must be measured into `manifest.json` before anything is cut from it — a guessed grid
that looks right is exactly how a kitbash ends up half a pixel off.

`node tools/catalogue-assets.mjs --check` fails when the catalogue is stale.
